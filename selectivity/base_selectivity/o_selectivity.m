function [ VoltageGradient, CurrentGradient, VOffset, IOffset, ResConcs,CapConcs, VoltageOffsets, CurrentOffsets, No,capSol,Resistance,Suppressed ] = o_selectivity( cIDs,varargin)
...
for j = 1:n
    ...
    if E.getReservoirConc() <=  maxResConc  && E.getReservoirConc() >=  minResConc     
        if E.getResistance ~= 0
            Resistance(i) = E.getResistance();% - Offsets(j);
        else
            Resistance(i) = nan;
        end
        if E.getVoffset ~= 0
