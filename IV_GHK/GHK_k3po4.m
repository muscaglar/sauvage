function [newIV, P] = GHK_k3po4( IV, ConcI, Conc0 )
...
n = max(size(V));
P = [1, 1];
iC = ( (1 .* Fc.* Fc .* P(1) .*V) ./ (8.3145.*300) )  .*  (  (ConcI - Conc0 .* exp(1.*Fc.*V ./8.3145.*300))  ./  ( 1 - exp(1.*Fc.*V ./8.3145.*300) ) );
iA = ( (9 .* Fc.^2 .* P(2) .*V) ./ (8.3145.*300) )  .*  (  (ConcI - Conc0 .* exp(-3.*Fc.*V ./8.3145.*300))  ./  ( 1 - exp(-3.*Fc.*V ./8.3145.*300) ) );
GHK = [iC iA ones(n,1)];
