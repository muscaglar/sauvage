function [newIV,P] = GHK_duo( IV, ConcI, Conc0 )
...
n = max(size(V));
P = [1, 1];
iC = ( (4 .* Fc.* Fc .* P(1) .*V) ./ (8.3145.*300) )  .*  (  (ConcI - Conc0 .* exp(2.*Fc.*V ./8.3145.*300))  ./  ( 1 - exp(2.*Fc.*V ./8.3145.*300) ) );
...
iC = @(PositiveIonPermeability,V) ( (4 .* Fc.* Fc .* PositiveIonPermeability .*V) ./ (8.3145.*300) )  .*  (  (ConcI - Conc0 .* exp(2.*Fc.*V ./8.3145.*300))  ./  ( 1 - exp(2.*Fc.*V ./8.3145.*300) ) );
...
newIV = [(V).*1e3, Ic+Offset, It+Offset];
...
GHK(isnan(GHK)) = 0;
CostFunc  =  nansum((I - GHK * [abs(P(1)) ; abs(P(2)); P(3)] ).^2 );
